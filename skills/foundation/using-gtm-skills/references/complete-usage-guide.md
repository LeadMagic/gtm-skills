...
# Complete gtm-skills Usage Guide
...
| # | System | Installer key | Project install path / method |
|---|---|---|---|
| 1 | Claude Code | `claude` | Native plugin; `.claude/skills/<skill-name>/` fallback |
| 2 | GitHub Copilot / VS Code | `copilot` / `vscode` | `.github/skills/<skill-name>/` via `gh skill` |
| 3 | Codex | `codex` | `.agents/skills/<skill-name>/` via `gh skill` |
| 4 | Cursor | `cursor` | `.agents/skills/<skill-name>/` via `gh skill` |
| 5 | Gemini CLI | `gemini` | `.agents/skills/<skill-name>/` via `gh skill` |
| 6 | OpenCode | `opencode` | `.agents/skills/<skill-name>/` via `gh skill` |
...
For a reviewed one-skill install, use `gh skill preview` followed by `gh skill
install LeadMagic/gtm-skills <category>/<skill-name> --agent <agent> --scope
project`. Pin a published release or commit when repeatability matters.
