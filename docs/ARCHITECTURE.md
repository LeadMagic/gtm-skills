...
└── skills.lock                     # SHA-256 manifest for every packaged skill file
```
...
- `skills.lock`
- Skill-local copies of directly referenced root catalogs (marked as generated)

Additional cross-repo catalogs:

- `references/README.md` — browsable index of the shared reference library
- `references/skill-index-master.md` — generated one-page map of every category and skill
...
`npm run build` runs the catalog generators, materializes portable shared references, and refreshes `skills.lock`.
...
`skills.lock` inventories every packaged `SKILL.md`, reference, template, script, and asset with its repository-relative path, SHA-256 digest, byte size, and kind. Consumers and CI can detect both changed bytes and missing or extra packaged paths.
...
- Artifact ownership, substance, portability, and executable-script checks.
- Complete `skills.lock` SHA-256, byte-size, kind, and path-set consistency.
- Installer dry-runs for all supported targets plus the curated Claude installer.
